using DataFrames, CSV, DecisionTree, Random, Statistics
using ScikitLearn.CrossValidation: KFold, cross_val_score
...
train_df.Age = coalesce.(train_df.Age, median(skipmissing(train_df.Age)))
...
y = train_df[:, :Survived]
X = convert(Matrix, train_df[:, Not(:Survived)])

# Train a Random Forest classifier and print accuracy
model = DecisionTree.RandomForestClassifier(n_trees=100, min_samples_split=10, max_depth=10)
